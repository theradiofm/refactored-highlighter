...
var _gInterface = {
  create: function(gSet){
        ...
        if (!$('.gearWrap').hasClass('inline') ) { _gMiniPlayer.create(gSet); };
  ...
  show: function(){
    ...
    if (!_gInterface.open || $('.stage').css('display')=='none') {  //double flag whenever one fails
      ...
      if (!$('.gearWrap').hasClass('inline') ) { _gInterface.disableScroll(); }
  ...
  disableScroll: function(){
    if (window.addEventListener) {
     ...
     if (!$('.gearWrap').hasClass('inline')) { $('.stage').css({'display':'none'}); } // hide the whole chunk, gains some cpu power
